...
- :bug: fix: 修复向下翻页时文字被截断的行（如只显示 20%）被直接翻过的问题；判定"行是否显示完整"时减去行间隙，只有完整显示的行才会翻过去，被截断的行整体带到下一页顶部完整显示
- :bug: fix: 向下翻页相位对齐：新视口按实测行距滚动，视口底部落在行间隙内，每页恰好显示整数行完整文字，页底不再截出无法阅读的残行（极端相位错位时最多残留少量截断，仍由下一页承接）
